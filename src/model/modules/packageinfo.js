...
const mutations = {
  UPDATE_SURE: function(state, bool) {
    //设置协议选中与否
    state.protocolSure = bool;
  },
  UPDATE_TWOTAGS: function(state, bool) {
    //是否展开发票和messagebox
    state.infoTwoTags = bool;
  },
  UPDATE_ORDERTIP: function(state, bool) {
    //近期已购买保养套餐
    state.orderTip = bool;
  },
  SET_STORE_INFO: function(state, params) {
    //设置购买套餐的店铺信息
  ...
  SET_RECEIPT_INFO: function(state, params) {
    //设置购买套餐的店铺信息
    if ($.isEmptyObject(params)) {
      state.receiptInfo = {};
    } else {
  ...
  UPDATE_USER_INFO: function(state, params) {
    //设置购买套餐的用户信息
    if ($.isEmptyObject(params)) {
      state.userInfo = {};
    } else {
  ...
  SET_CARLIST: function(state, params) {
    //设置可选车型列表
    state.carList = params;
  },
  SET_CARLIST1: function(state, params) {
    //设置可选车型列表
    state.carList1 = params;
  },
  SET_MODULE_INFO: function(state, params) {
    //设置购买套餐的车型信息
    if ($.isEmptyObject(params)) {
      state.modelInfo = {};
    } else {
  ...
  SET_SETINFO: function(state, params) {
    //设置套餐信息
    if ($.isEmptyObject(params)) {
      state.setInfo = {};
    } else {
  ...
  SET_SETDETAIL: function(state, params) {
    //设置套餐详情
    if ($.isEmptyObject(params)) {
      state.setDetail = {};
    } else {

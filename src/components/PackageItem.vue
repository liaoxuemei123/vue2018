...
export default {
  ...
  computed: {
    ...mapState({
      ...
      allConfig: ({ pageconfig }) => pageconfig.config,
      currentBis: ({ pageconfig }) => pageconfig.currentBis
    })
  },
  methods: {
    viewDetail: function() {
      var item = this.item;
      const bisiness = this.$parent.$parent.$parent.bisinessItems[
        this.$parent.$parent.$parent.activeBusiness
      ].wbyId;
      ...
      if (this.modelInfo.vehicleModel && this.modelInfo.pickerModel) {
        ...
        Tool.post(
          "packagecount",
          { packageId: item.wbpId, packageName: item.wbpName, isUniversal: item.wbpSfqgty },
          data => {}
        );
      } else {

<template>
  <div class="page-container">
    <div class="page personinfo-page" flex="dir:top box:justify">
      <nav-bar title="填写个人信息" rightIcon="icon-home" :onRight="goHome" :goBack="goBack.bind(this)" />
      <div class="page-content">
        <div class="input-container" v-show="pageConfig.fileds.length > 0" v-for="(item,index) in pageConfig.fileds" :key="index">
          ...
          <div class="input-control" v-if="refereeType == 1 && item == 'referee' && pageConfig.tags[index] == '1'">
            <inp-com title="推荐人" placeholder="填写推荐人手机号" :value="refereePhone" :onBlur="updatePhone.bind(this)" />
          </div>
          <div class="input-control" v-if="refereeType == 0 && item == 'referee' && pageConfig.tags[index] == '1'">
            <inp-com title="推荐服务门店" placeholder="请选择服务门店" :rightArrow="true" :readonly="true" :onClick="goStore.bind(this)" :value="storeName" />
          </div>
          ...
          <div class="input-control-custom" v-if="item == 'tel' && pageConfig.tags[index] == '1'" flex="dir:left cross:center box:justify" v-show="isValidate">
            ...
            <div class="button" flex="dir:left cross:center main:right">
              <span ref="onValiCode">
                <span ref="residueTime">60</span>秒后重发</span>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      modelInfo: ({ packageinfo }) => packageinfo.modelInfo,
      pUserInfo: ({ packageinfo }) => packageinfo.userInfo,
      pageSetting: ({ pageconfig }) => pageconfig.currentBis,
      userVehicle: ({ packageinfo }) => packageinfo.userVehicle,
      refereeStore: ({ packageinfo }) => packageinfo.refereeStore,
      storeInfo: ({ packageinfo }) => packageinfo.storeInfo,
      getedVehiche: ({ mixin }) => mixin.getedVehiche,
      receiptInfo: ({ packageinfo }) => packageinfo.receiptInfo,
      orderTip: ({ packageinfo }) => packageinfo.orderTip,
      twoTags: ({ packageinfo }) => packageinfo.infoTwoTags
    })
  },
  activated: function() {
    ...
    if (this.refereeStore && this.refereeStore.storeName) {
      ...
      this.storeName =
        this.refereeStore.storeName == "任意服务门店" ? "" : this.refereeStore.storeName;
      ...
      if (this.userVehicle.length <= 0 && !this.getedVehiche) {
        ...
        if (this.userInfo.motorId || this.needVerify == true) {
          ...
          if (this.userVehicle.length > 0) {
            ...
            if (
              this.userVehicle.length > 0 &&
              this.userVehicle[0].vehicle_type_id == this.modelInfo.id
            ) {
              this.needVerify = false;
  ...
  methods: {
    ...
    nextPage: function() {
      if (
        !this.userInfo.vin &&
        this.pageConfig.tags[this.pageConfig.fileds.indexOf("vin")] == "1"
      ) {
        Toast({
          message: "请输入车架号",
          duration: 1000
      ...
      if (
        (this.userVehicle.length > 0 &&
          this.userVehicle[0].vehicle_type_id == this.modelInfo.id &&
          this.userInfo.vin == this.userVehicle[0].vin) ||
        (this.selectCarData && this.userInfo.vin == this.selectCarData.vin)
      ) {
      } else {
        if (
          !this.userInfo.motorId &&
          this.pageConfig.tags[this.pageConfig.fileds.indexOf("motorId")] == "1"
        ) {
          Toast({
            message: "请输入发动机号",
            duration: 1000
      ...
      if (
        ...
        this.pageConfig.tags[this.pageConfig.fileds.indexOf("contact")] == "1"
      ) {
        Toast({
          message: "请输入姓名",
          duration: 1000
      ...
      if (
        !this.userInfo.tel &&
        this.pageConfig.tags[this.pageConfig.fileds.indexOf("tel")] == "1"
      ) {
        Toast({
          message: "请输入手机号",
          duration: 1000
      ...
      if (this.pageConfig.tags[this.pageConfig.fileds.indexOf("motorId")] == "1") {
        ...
        new Promise((res, rej) => {
          if (
            ...
            res({
              data: {
                buyCarDate: this.selectCarData
                  ? this.selectCarData.buyCarDate
                  : this.userVehicle[0].buyCarDate,
          ...
          //                 })
          //             }else{
          //                 this.$router.push({name:'confirmorder',params: { page: "personinfo" }});
          ...
          // }else{
          this.userInfo.engineNo = pData.data.engineNo;
          this.userInfo.mileage = Math.ceil(
            (+new Date() -
              (pData.data.buyCarDate ? +new Date(pData.data.buyCarDate) : new Date())) /
              (1000 * 60 * 60 * 24)
          );
          this.userInfo.buyCarDate = pData.data.buyCarDate;
          this.updateUserInfon(this.userInfo);
                ...
                Tool.get(
                  ...
                  data => {
                    if (data.code == 200) {
                      ...
                      //没有则不做操作
                      this.$router.push({ name: "confirmorder", params: { page: "personinfo" } });
                      Tool.localItem("vehicleInfo", {
                        vin: this.userInfo.vin,
                        engineNo: this.userInfo.motorId,
                        userName: this.userInfo.contact
                      });
                    }
                  }
                );
    ...
    submitReferee: function() {
      if (this.refereeType == 0) {
        this.setUserInfo({
          ...
          referee: this.storeName,
          refereeId: this.storeId
        });
      } else if (this.refereeType == 1) {
        this.setUserInfo({
          refereeType: this.refereeType,
          referee: this.refereePhone,
          refereeId: ""
        });
      } else {

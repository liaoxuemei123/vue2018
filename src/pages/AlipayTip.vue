<template>
  <div class="page-container">
    <div class="wechat-page page" flex="dir:top box:first">
      <nav-bar title="支付提示" :goBack="backToHome" />
  ...
  activated: function() {
    ...
    if (location.hash.indexOf("error") != -1) {
      ...
      if (ua.indexOf("micromessenger") != -1) {
        ...
        var hrefString = this.goto != "" ? decodeURIComponent(this.goto) : "/alipaytip#error";
        if (hrefString == "/alipaytip#error") {
          location.hash = hrefString;
        } else {
          location.href = hrefString;
        }
        // query:{wbyQd:String(to.query.wbyQd)}
        if (location.hash.indexOf("error") != -1) {
          // alert('参数错误，请检查');
          Toast({
            message: "参数错误，请检查",

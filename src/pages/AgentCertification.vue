<template>
  <div class="page-container">
    <div class="index-page page" flex="dir:top box:justify">
      <nav-bar title="经纪人认证" :onRight="onRight" rightContent="说明" />
      <div class="page-content">
        ...
        <div class="qrCode" v-if="hasCode" style="position:relative;">
          <div style="text-align: center;font-size: 1rem;color:white;position:absolute;top:0.1rem;width: 100%;" @click="myrecommendorder">
            <i style="text-decoration: underline">查看我的推荐订单</i>
          </div>
          <div class="box" v-if="hasCode">
            <div style="text-align: center;font-size: 1rem;color:white;position:absolute;top:-1.25rem;width: 100%;">
              <div>我的推荐二维码</div>
            </div>
            <img :src="qrCodeStr" alt="">
          </div>
      ...
      <div class="button-control" v-show="!hasCode">
        <btn-com title="确定" :onClick="sureCert" background="#00bffe" />
      </div>
    </div>
...
export default {
      ...
      data => {
        if (data.code == 0) {
          ...
          var Str = encodeURIComponent(
            "http://47.92.32.44:9990/maintainpackage/#/maintainset?wbyQd=JJRFX&agentLogo=" +
              data.data
          );
  ...
  beforeRouteEnter: (to, from, next) => {
    ...
    if (to.query.dealerId) {
      ...
      if (
        from.name == "login" ||
        from.name == "myrecorder" ||
        from.name == "agentexplain" ||
        !from.name
      ) {
      } else Tool.removeLocalItem("dealerId");
    }
    if (to.query.oid) {
      //如果有单独传的oid则表示由管理那面直接跳过来不用管其他

              ...
              <div class="set-detail">
                ...
                <div class="line" flex="dir:left cross:center">
                  <span class="price-range">
                    <span class="doller">￥</span>{{showPrice | priceFilter}}</span>
                  <span class="price-range" v-if="rangePrice > 0">&nbsp;-&nbsp;
                    <span class="doller">￥</span>{{rangePrice | priceFilter}}</span>
                  <span class="origin-price">
                    <span class="doller" v-if="deletePrice">￥</span>{{deletePrice | priceFilter}}</span>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    nextPage: function() {
      if (
        !this.setDetail.price &&
        this.pageConfig.tags[this.pageConfig.fileds.indexOf("meal")] == "1"
      ) {
        Toast("请选择机油");
        return false;
      }
      if (
        this.setInfo.wbpSfqgty == 2 &&
        !this.storeInfo.id &&
        this.pageConfig.tags[this.pageConfig.fileds.indexOf("store")] == "1"
      ) {
        Toast({
          message: "请选择服务门店",
          duration: 1000
    ...
    selectedMeal: function(index) {
      ...
      this.showPrice =
        this.setMealList.length > 0 ? this.setMealList[this.selectMeal].wbplXSPrice : 0;
      this.rangePrice = 0;
      this.deletePrice =
        this.setMealList.length > 0 ? this.setMealList[this.selectMeal].wbplPrice : 0;
      ...
      if (this.setMealList.length > 0) {
        ...
        this.setDetail.mealName =
          this.setMealList[this.selectMeal].wbplJyName + this.setMealList[this.selectMeal].wbplJyXh;
      }

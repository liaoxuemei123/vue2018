<template>
  <div class="page-container">
    <div class="page order-pay-page" flex="dir:top box:justify">
      <nav-bar title="确认购买" :goBack="goBack" />
...
export default {
  ...
  methods: {
    pay: function() {
      ...
      Tool.get(
        "checkOrder",
        { orderId: this.orderNo, total_fee: this.orderInfo.orderPrice },
        data => {
          if (data.code == 200) {
            if (this.paymentMode == 1) {
              if (!Tool.isWeChat()) {
                ...
                window.location.href =
                  "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx22b47ca6974f3e71&redirect_uri=https://cloud.mall.changan.com.cn%2Fmaintainpackage%2F%23%2Forderpay%2F" +
                  self.orderNo +
                  "&response_type=code&scope=snsapi_base&state=" +
                  self.orderNo +
                  "," +
                  self.orderInfo.packageName +
                  "," +
                  self.qd +
                  "#wechat_redirect";
              }
              ...
              if (Tool.isWeChat()) {
                var alipayUrl = encodeURIComponent(
                  Tool.target +
                    "aliPay?out_trade_no=" +
                    this.orderNo +
                    "&total_amount=" +
                    this.orderInfo.orderPrice +
                    "&subject=" +
                    this.orderInfo.packageName +
                    ""
                );
              ...
              window.location.href =
                "https://cloud.mall.changan.com.cn/maintenance-plug/unionPay/frontConsume?orderId=" +
                this.orderNo;
            }
          } else {
            Toast({
    ...
    weChatH5: function() {
      const total_fee = (this.orderInfo.orderPrice - 0) * 100,
        ...
        redirect_url = encodeURIComponent(
          "https://cloud.mall.changan.com.cn/maintainpackage/#/wechath5?orderNo=" +
            this.orderNo +
            "&total_fee=" +
            total_fee
        );
      const url = `${
        Tool.target
      }weixinH5pay?total_fee=${total_fee}&body=${body}&OutTradeNo=${orderNo}&ip=${ip}&redirect_url=${redirect_url}`;
    ...
    .order-info {
      ...
      &:after {
        ...
        background-image: -webkit-gradient(
            ...
            color-stop(0.5, #efefee),
            to(#efefee)
          ),
          -webkit-gradient(linear, 50% 0, 100% 100%, from(transparent), color-stop(0.5, transparent), color-stop(0.5, #efefef), to(#efefef));

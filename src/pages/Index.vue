<template>
  <div class="page-container">
    <div class="index-page page" flex="dir:top box:justify">
      <nav-bar title="添加预约订单" rightIcon="icon-history" rightContent="预约历史" :buttonWidth=0.28 :onRight="goHistory" />
      <div class="page-content">
        <mt-datetime-picker ref="datepicker" type="datetime" v-model="pickerV" @confirm="onDateConfirm" month-format="{value}月" date-format="{value}日" hour-format="{value}时" minute-format="{value}分" :startHour='startHour' :endHour='endHour' :startDate="startDate" :endDate="endDate">
        </mt-datetime-picker>
        ...
        <div class="input-control">
          ...
          <transition name="drop-down">
            <div class="explain" v-if="subscribeInfo.fcmc.exchange.length>0">
              <div class="atention" flex="dir:left">保养推荐：
                <div class="red">以下保养项目按照官方保养守则推荐具体以到店为准</div>
              </div>
              <div class='fcmc-list'>
      ...
      <div class="button-control">
        <btn-com title="确定预约" :onClick="addOrder" background="#00bffe" />
      </div>
    </div>
...
export default {
  ...
  activated: function() {
    if (Tool.localItem("userCache")) {
      ...
      if (JSON.parse(Tool.localItem("userCache")).mobile) this.subscribeInfo.phone = this.mobile;
    }
  ...
  methods: {
    ...
    onDateConfirm: function(val) {
      ...
      if (
        (new Date(val).getHours() >= 17 && new Date(val).getMinutes() > 30) ||
        new Date(val).getHours() >= 18
      ) {
        Toast({
    ...
    ...mapMutations(["updateSubscribeInfo"])
  },
  beforeRouteLeave: function(to, from, next) {
    if (
      !Tool.localItem("userCache") &&
